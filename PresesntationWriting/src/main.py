...
def get_presentation(buyer:User,seller:User,client_requirement:str , service_dept:list,
                     additonal_info : str  ):
    ...
    generate_presentation(filename  = 'output.txt',logo_url=logo,logo_url_2 = state['buyer'].logo ,colors = colors,output_format='pdf')
    generate_presentation(filename='output.txt',logo_url = logo,logo_url_2 = state['buyer'].logo,colors=colors,output_format='html')
    return True
